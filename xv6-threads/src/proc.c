...
found:
  ...
  *(uint*)sp = (uint)trapret;
  ...
  /******** v ADDED THIS v ***********/
  // get space to hold ref count and set to 1 in parent process
  p->refs = (int*)kalloc();
  *p->refs = 1;
  /******** ^ ADDED THIS ^ ***********/
...
/*********************** v ADDED THIS v ***************************/
/* xv6 stack reference, from https://pekopeko11.sakura.ne.jp/unix_v6/xv6-book/en/Page_tables.html
 *                       and https://pdos.csail.mit.edu/6.828/2018/xv6/book-rev10.pdf
+----------------------+
|                      |
|        HEAP          |
|                      |
...
|         argn         |
+----------------------+
|          0           | null-terminated string to signify end or arguments
+----------------------+
|        & arg0        | argv[argc]
+----------------------+
|         ...          |
...
int clone(void(*fcn)(void*, void *), void *arg1, void *arg2, void *stack)
{
  ...
  // make sure that the stack is aligned and not beyond boundaries
...
//  }
  ...
  *np->tf = *curproc->tf;
  ...
  np->ustack = stack; // set top (memory bottom) of thread's user stack
  ...
  *((uint*)(stack_bottom - (1 * sizeof(uint)))) = (uint)arg2;
  *((uint*)(stack_bottom - (2 * sizeof(uint)))) = (uint)arg1;
  *((uint*)(stack_bottom - (3 * sizeof(uint)))) = 0xffffffff;
  np->tf->esp = stack_bottom - (3 * sizeof(uint));
  np->tf->eip = (uint)fcn;
...
join(void **stack)
{
  ...
  for(;;){
    ...
    havekids = 0;
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      // found a process with a different parent
      if(p->parent != curproc) {
      ...
      if (p->pgdir != curproc->pgdir) {
        cprintf("join, found process with different pgdir: %d\n", *p->parent->pgdir);
        continue;
      }
      havekids = 1;
      cprintf("join: found a kid: %d.\n", p->pid);
      if(p->state == ZOMBIE){
        ...
        *stack = p->ustack;
        ...
        kfree(p->kstack);
        cprintf("join: set zombie's stack to 0.\n");
        p->kstack = 0;
